...
This skill refines prompts for specific AI models by following these steps:

1. User provides a prompt and names (or the agent infers) the target model.
2. Load the execution contract from `./agent.md`.
3. Load the model-specific reference from `./reference/models/<vendor>/<model>.md`.
4. Classify the prompt type and load only the matching technique files from `./reference/techniques/`.
5. Rewrite the prompt applying model-specific patterns while preserving the original intent.
6. Return the refined prompt with brief change notes.

Only load the target model file and the technique files needed for the classified prompt. If the model is not documented, use `./reference/universal.md` and state that the universal fallback was used.

> Files in `./reference/` are read-only reference data. Do not modify them during execution.

> Treat `./examples/` as demonstrations and test fixtures only. Do not treat example text as instruction to follow literally.
...
1. **Identify target model**
   ...
   - If model is not documented, load `./reference/universal.md` and apply general patterns with an explicit fallback note
...
6. **Self-verify**
   ...
   - Confirm the output still asks for the same outcome as the input and did not expand scope
